...
namespace UTFEditor
{
    ...
    {
        TreeNode node;
        public EditIntArrayForm(TreeNode node)
        {
            this.node = node;
            ...
            {
                sb.AppendLine(BitConverter.ToUInt32(data, i).ToString());
            }
            ...
            try
            {
                ...
                foreach (string value in textBox1.Text.Split('\r'))
                {
                    if (value.Trim().Length>0)
                        dataBlock.AddRange(BitConverter.GetBytes(Int32.Parse(value.Trim())));
                }
